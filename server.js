...
// Answer survey neutrally and submit — once answered it never shows again
async function killSurvey(page){
  const result = await page.evaluate(()=>{
    const actions = [];

    // Find all radio buttons / rating options and select middle one
    const radioGroups = {};
    ...
    for(const [name,radios] of Object.entries(radioGroups)){
      // Pick the middle option (neutral)
      const mid=Math.floor(radios.length/2);
      radios[mid].click();
      actions.push(`radio "${name}" → option ${mid+1}/${radios.length}`);
    }

    // Find rating stars/numbers and click middle one
    const ratingEls=document.querySelectorAll("[class*=rating],[class*=star],[class*=score]");
    for(const container of ratingEls){
      const items=container.querySelectorAll("input,span,div,a,button,label");
      if(items.length>1){
        const mid=Math.floor(items.length/2);
        items[mid].click();
        actions.push(`rating → item ${mid+1}/${items.length}`);
        break;
      }
    }

    // Find textarea/text inputs and fill with neutral text
    for(const el of document.querySelectorAll("textarea,input[type=text]")){
      if(el.offsetParent===null)continue;
      el.value="N/A";
      el.dispatchEvent(new Event("input",{bubbles:true}));
      actions.push("filled textarea");
    }

    // Find and click Submit button
    const submitKw=["submit","send","done","finish","complete","ok","continue","next"];
    for(const el of document.querySelectorAll("button,input[type=submit],input[type=button],a")){
      ...
      if(submitKw.some(k=>txt===k||txt.includes(k))&&el.offsetParent!==null&&txt.length<20){
        el.click();
        actions.push(`submitted: "${txt}"`);
        return actions;
      }
    }

    // If no submit found, try × close button
    for(const el of document.querySelectorAll("*")){
      const txt=(el.textContent||"").trim();
      if((txt==="×"||txt==="✕")&&el.offsetParent!==null){
        el.click();
        actions.push("closed ×");
        return actions;
      }
  ...
  if(result&&result.length){
    console.log(`  Survey handled: ${result.join(", ")}`);
    await wait(2000);
  } else {
...
async function getRoster(username,password){
  ...
  try{
    ...
    // ── Login ────────────────────────────────────────────────────────
    console.log("  Login...");
    await page.goto(CWP_LOGIN,{waitUntil:"domcontentloaded",timeout:60000});
    await page.waitForSelector("input[type=password]",{timeout:15000});

    await page.evaluate((u,p)=>{
      for(const el of document.querySelectorAll("input")){
        const n=(el.name||el.id||"").toLowerCase();
        if((el.type==="text"||n.includes("user"))&&!n.includes("pass")){
    ...
    if(url1.toLowerCase().includes("login")){
      ...
      if(body.includes("invalid")||body.includes("incorrect"))throw new Error("LOGIN_FAILED");
    }

    // ── Dismiss survey popup aggressively ────────────────────────────
    console.log("  Killing survey popup...");
    await killSurvey(page);
    await killSurvey(page); // double tap

    // ── Navigate directly to the PDF roster page ──────────────────────
    // This bypasses the survey entirely — go straight to the URL
    console.log("  Navigating to PDF roster page...");
    await page.goto(CWP_PDF,{waitUntil:"networkidle2",timeout:60000});
    await killSurvey(page);

    // Wait for actual roster content
    let text = "";
    const gotDuties = await page.waitForFunction(()=>{
      const t=document.body.innerText;
      return t.includes("RGD")||t.includes("JQ")||t.includes("PHO")||
             (t.includes("BLH")&&t.includes("WED"));
    },{timeout:20000}).then(()=>true).catch(()=>false);

    text = await page.evaluate(()=>document.body.innerText);
    console.log(`  PDF page: ${text.length} chars | hasDuties: ${gotDuties}`);

    if(!gotDuties){
      // Try clicking the Roster nav link directly
      console.log("  Clicking Roster nav link...");
      await page.goto(CWP_MAIN,{waitUntil:"networkidle2",timeout:60000});
      await killSurvey(page);

      // Click the "Window Roster" or "Individual Roster" link if visible
      const clicked = await page.evaluate(()=>{
        const links=Array.from(document.querySelectorAll("a"));
        for(const a of links){
          const txt=(a.textContent||"").toLowerCase();
          if(txt.includes("window")||txt.includes("individual")||txt.includes("pdf")){
      ...
      if(clicked){
        ...
        await page.waitForFunction(()=>{
          const t=document.body.innerText;
          return t.includes("RGD")||t.includes("JQ")||t.length>5000;
        },{timeout:20000}).catch(()=>{});
      }

      text = await page.evaluate(()=>document.body.innerText);
      console.log(`  Final page: ${text.length} chars`);
    }
...
function parse(text){
  ...
  for(const line of text.split(/[\n\r]+/).map(l=>l.trim()).filter(Boolean)){
    ...
    if(FR.test(line)){
      ...
      duties.push({type:"flight",date:cur,flightNo:fm[1],from:iatas[0]||"AKL",to:iatas[1]||null,
        signOn:times[0]||null,ata:times[1]||null,signOff:times[times.length-1]||null});
    }else{
      for(const code of DC){
        if(new RegExp(`\\b${code}\\b`).test(line)){
          const times=gt(line);
          duties.push({type:"duty",date:cur,code,signOn:times[0]||null,
  ...
  return{crewName:nm?nm[1]:"",empId:nm?nm[2]:"",
    blh:bm?`${bm[1]}:${bm[2]}`:"0:00",month,year,duties,
