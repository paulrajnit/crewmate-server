...
// Answer survey neutrally — fire and forget, never blocks navigation
async function killSurvey(page){
  try{
    const result = await page.evaluate(()=>{
      const actions=[];
      // Select middle radio option on each question
      const groups={};
      ...
      for(const [n,r] of Object.entries(groups)){
        r[Math.floor(r.length/2)].click();
        actions.push(`radio ${n}`);
      }
      // Fill any visible textareas
      for(const el of document.querySelectorAll("textarea")){
        if(el.offsetParent!==null){el.value="N/A";actions.push("textarea");}
      }
      // Click submit/close — do NOT trigger navigation, just click
      const kw=["submit","send","done","finish","ok","close","×","✕"];
      for(const el of document.querySelectorAll("button,input[type=submit],input[type=button],a,span,div")){
        const txt=(el.textContent||el.value||"").trim().toLowerCase();
        if(kw.some(k=>txt===k||txt.includes(k))&&el.offsetParent!==null&&txt.length<20){
          el.click();actions.push(`click:${txt}`);break;
        }
      }
      return actions;
    });
    if(result&&result.length)console.log(`  Survey: ${result.join(", ")}`);
  }catch(e){}
  // Short fixed wait — no waitForNavigation
  await wait(1000);
}
